...
def loaded_model(json_file_path, weights_path):

    json_file = open(os.path.join(MODEL_PATH, json_file_path), 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)

    # load weights into new model
    loaded_model.load_weights(os.path.join(MODEL_PATH, weights_path))
    #loaded_model(weights_path, custom_objects={'AttentionDecoder': AttentionDecoder})
